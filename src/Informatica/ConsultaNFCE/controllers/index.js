...
export async function ufToCodigo(uf) {
  const map = {
...
class ConsultaNfeController {
  ...
  async validarConsultar(req, res) {
    try {
      ...
      for (const row of vendas) {
        ...
        const TPAMB = row.NFE_INFNFE_IDE_TPAMB || 2 || 1;
        const MOD = row.NFE_INFNFE_IDE_MOD || "65" || "55";
        ...
        try {
          ...
          resultados.push({ IDVENDA, UF, CHAVE, TPAMB, MOD, CSTAT: cstat, XML: xml });
        } catch (e) {
          resultados.push({ IDVENDA, UF, CHAVE, TPAMB, MOD, error: e.message });
        }
      }
  ...
  async consultaNFce(req, res) {
    try {
      ...
      const payload = await gerarXML(vendaData);
      const codigoUF = await this.ufToCodigo(payload.ide.cUF);

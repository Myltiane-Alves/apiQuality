...
class ConsultaStatusNfeController {
  async validarConsulta(req, res) {
    try {
      ...
      // 1) CERTIFICADO
      // =========================
      const senhaCert = process.env.SENHA;
      const certOptions = getCertOptions(senhaCert);
      ...
      if (!vendas) {
        ...
        const apiUrl =
          `http://164.152.245.77:8000/quality/concentrador_homologacao` +
          `/api/venda/valida-venda-contingencia.xsjs?${params.toString()}`;
      ...
      if (!Array.isArray(vendas)) {
        vendas =
          ...
          vendas?.rows ??
          vendas?.data?.rows ??
          Object.values(vendas || {}).find(Array.isArray);
      }

      if (!Array.isArray(vendas) || vendas.length === 0) {
        return res.status(400).json({
          error: "Nenhuma venda para consultar"
        });
      }
